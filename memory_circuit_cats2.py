...
def main(args):
    ...
    cats = np.arange(cn, cn+max(np.count_nonzero(Hx, axis=1)))
        ...
        def simulate(self):
            ...
            for _ in range(1, self.num_rounds+1):
                ...
                if (not self.adaptive):
                    ...
                    # if (_ > 1) and ((_ - 1) % 10 == 0):
                    #     self.curr_z_checks = np.arange(qedm, cm)
                    #     self.curr_x_checks = np.arange(qedm, cm)
    ...
    for ii in range(num_shots):
        ...
        num_CNOTs = (str(c).count("CX") + str(c).count("CZ")) / num_rounds
