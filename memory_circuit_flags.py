...
def main(args):
    ...
    x_flags = np.arange(cn+cmx+cmz,cn+cmx+cmz+qedm)
    z_flags = np.arange(cn+cmx+cmz+qedm,cn+cmx+cmz+qedm+qedm)
    ...
    precomputed_z_qed_checks_circuits = np.empty(qcode.qedzm, dtype=object)
    for z in np.arange(qcode.qedzm):
        c = stim.Circuit()
        gen_qbts = data_qbts[np.where(Hz[z])[0]]

        c.append("H", z_flags[z])
        c.append("DEPOLARIZE1", z_flags[z], qubit_error_rate)

        # ONLY WORKS FOR [[4,2,2]] CODE CURRENTLY
        c.append("CNOT", [gen_qbts[0], z_checks[z]])
        c.append("DEPOLARIZE2", [gen_qbts[0], z_checks[z]], qubit_error_rate)

        c.append("CNOT", [z_flags[z], z_checks[z]])
        c.append("DEPOLARIZE2", [z_flags[z], z_checks[z]], qubit_error_rate)

        c.append("CNOT", [gen_qbts[1], z_checks[z]])
        c.append("DEPOLARIZE2", [gen_qbts[1], z_checks[z]], qubit_error_rate)
        c.append("CNOT", [gen_qbts[2], z_checks[z]])
        c.append("DEPOLARIZE2", [gen_qbts[2], z_checks[z]], qubit_error_rate)

        c.append("CNOT", [z_flags[z], z_checks[z]])
        c.append("DEPOLARIZE2", [z_flags[z], z_checks[z]], qubit_error_rate)

        c.append("CNOT", [gen_qbts[3], z_checks[z]])
        c.append("DEPOLARIZE2", [gen_qbts[3], z_checks[z]], qubit_error_rate)

        c.append("H", z_flags[z])
        c.append("DEPOLARIZE1", z_flags[z], qubit_error_rate)

        precomputed_z_qed_checks_circuits[z] = c

    def prepare_z_qed_checks():
        c = stim.Circuit()
        for z in np.arange(qcode.qedzm):
            c += precomputed_z_qed_checks_circuits[z]
        return c

    precomputed_x_qed_checks_circuits = np.empty(qcode.qedxm, dtype=object)
    for x in np.arange(qcode.qedxm):
        c = stim.Circuit()
        gen_qbts = data_qbts[np.where(Hx[x])[0]]

        c.append("H", [x_flags[x], x_checks[x]])
        c.append("DEPOLARIZE1", [x_flags[x], x_checks[x]], qubit_error_rate)

        # ONLY WORKS FOR [[4,2,2]] CODE CURRENTLY
        c.append("CNOT", [x_checks[x], gen_qbts[0]])
        c.append("DEPOLARIZE2", [x_checks[x], gen_qbts[0]], qubit_error_rate)

        c.append("CNOT", [x_flags[x], x_checks[x]])
        c.append("DEPOLARIZE2", [x_flags[x], x_checks[x]], qubit_error_rate)

        c.append("CNOT", [x_checks[x], gen_qbts[1]])
        c.append("DEPOLARIZE2", [gen_qbts[1], z_checks[z]], qubit_error_rate)
        c.append("CNOT", [x_checks[x], gen_qbts[2]])
        c.append("DEPOLARIZE2", [gen_qbts[2], z_checks[z]], qubit_error_rate)

        c.append("CNOT", [x_flags[x], x_checks[x]])
        c.append("DEPOLARIZE2", [x_flags[x], x_checks[x]], qubit_error_rate)

        c.append("CNOT", [x_checks[x], gen_qbts[3]])
        c.append("DEPOLARIZE2", [gen_qbts[3], z_checks[z]], qubit_error_rate)

        c.append("H", [x_flags[x], x_checks[x]])
        c.append("DEPOLARIZE1", [x_flags[x], x_checks[x]], qubit_error_rate)

        precomputed_x_qed_checks_circuits[x] = c

    def prepare_x_qed_checks():
        c = stim.Circuit()
        for x in np.arange(qcode.qedxm):
            c += precomputed_x_qed_checks_circuits[x]
        return c
    ...
    class Simulation:
        ...
        def QED(self):
            def measure_z_qed_checks():
                ...
                c.append("X_ERROR", [z_flags[z_flag] for z_flag in np.arange(qcode.qedzm)], meas_error_rate)
                c.append("MR", [z_flags[z_flag] for z_flag in np.arange(qcode.qedzm)])
                c.append("X_ERROR", [z_flags[z_flag] for z_flag in np.arange(qcode.qedzm)], meas_error_rate)
                return c
            ...
            def measure_x_qed_checks():
                ...
                c.append("X_ERROR", [x_flags[x_flag] for x_flag in np.arange(qcode.qedxm)], meas_error_rate)
                c.append("MR", [x_flags[x_flag] for x_flag in np.arange(qcode.qedxm)])
                c.append("X_ERROR", [x_flags[x_flag] for x_flag in np.arange(qcode.qedxm)], meas_error_rate)
                return c
            ...
            for _ in range(1, self.num_rounds+1):
                ...
                if (not self.adaptive):
                    ...
                    if self.stab_type:
                        ...
                        self.z_syndrome_history[self.curr_round][:qcode.qedzm] = meas[-(4*qedm):-(3*qedm)]
                        curr_z_flags = meas[-(3*qedm):-(2*qedm)]
                        self.x_syndrome_history[self.curr_round][:qcode.qedxm] = meas[-(2*qedm):-qedm]
                        curr_x_flags = meas[-qedm:]

                        x_qed_synd_diff = self.x_syndrome_history[0][:qcode.qedxm] ^ self.x_syndrome_history[self.curr_round][:qcode.qedxm]
                ...
                if self.stab_type:
                    guessed_error = decode(self.x_syndrome_history[self.curr_round], 1, True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-r', default=30, help="Number of rounds")
