...
def get_data(concat, adaptive, qcode):
    ...
    lifetime_data = np.zeros(shape=(6,6), dtype=float)

    for ii, i in enumerate(range(1,7)):
        phys_error = round(i*0.001, 6)
        for jj, j in enumerate(range(1,7)):
            meas_error = round(j*0.001, 6)
...
def annotate_heatmap(ax, data):
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            ...
            text = ax.text(j, i, text, ha='center', va='center', color='white', fontsize=14)
...
concat1adaptive0 = get_data(0,1,qcode2)
...
im2 = axes[1].imshow(concat1adaptive0, cmap='viridis_r', origin='lower',  norm=LogNorm(vmin=vmin, vmax=vmax,))
im3 = axes[2].imshow(concat1adaptive1, cmap='viridis_r', origin='lower',  norm=LogNorm(vmin=vmin, vmax=vmax,))
...
axes[1].set_title(qcode2)
axes[2].set_title(qcode1)

# for i in range(3):
#     axes[i].set_xticklabels(['0.001', '0.002', '0.003', '0.004', '0.005', '0.006'])

positions = [0, 1, 2, 3, 4, 5]
labels = ['0.001', '0.002', '0.003', '0.004', '0.005', '0.006']
for i in range(3):
